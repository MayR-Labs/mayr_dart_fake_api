...
class PlaceholderReplacer {
  ...
  static Map<String, dynamic> replacePlaceholders(
    ...
    String processString(String value) {
      ...
      result = result.replaceAll(
        r'$userAvatar',
        'https://i.pravatar.cc/150?u=${_uuid.v4()}',
      );
      ...
      result = result.replaceAll(
        r'$sku',
        'SKU-${_random.nextInt(99999).toString().padLeft(5, '0')}',
      );

      // Color/Design placeholders
      result = result.replaceAll(
        r'$hexColor',
        '#${_random.nextInt(0xFFFFFF).toRadixString(16).padLeft(6, '0').toUpperCase()}',
      );
      ...
      result = result.replaceAll(
        r'$version',
        '${_random.nextInt(10)}.${_random.nextInt(20)}.${_random.nextInt(50)}',
      );
      ...
      for (final entry in customPlaceholders.entries) {
        // Token we are looking for inside the string (e.g. $customValue)
        final token = '\$${entry.key}';
        // Only invoke generator if the token actually appears in the string
  ...
  static String _replaceParameterizedPlaceholders(String value) {
    ...
    // $randomFloat(min,max)
    final randomFloatPattern = RegExp(
      r'\$randomFloat\((\d+\.?\d*),(\d+\.?\d*)\)',
    );
